...
int main() {
    ...
    int c, wlenght, max_used_lenght;
    FLAG state;

    wlenght = max_used_lenght = 0;
    state = OUT;
    ...
    while ((c = getchar()) != EOF) {
        ...
        if (state == IN) {
            if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) {
                ...
                if (words_lenght[wlenght] > max_used_lenght)
                    max_used_lenght = words_lenght[wlenght];
                state = OUT;
                wlenght = 0;
            }
        }
    ...
    for (int i = max_used_lenght; i > 0; i--) {
